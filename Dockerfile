...
FROM openjdk:11
#Env to have a defult profile in spring-app
ENV springProfile=local
...
ENTRYPOINT ["java","--class-path","app:app/lib/*","-Dspring.profiles.active=${springProfile}","com.tefnick.userservice.UserServiceApplication"]
...
#
#ENTRYPOINT ["java","-Dspring.profiles.active=${springProfile}","-jar","app.jar"]
